...
ws.on("message", (data) => {
  try {
    ...
    console.log(
      `\nServer: ${
        message.type === "info" ? message.message : JSON.stringify(message)
      }`
    );
    promptUser();
  } catch (error) {
...
function handleCommand(input) {
  ...
  switch (command.toLowerCase()) {
    ...
    case "win":
      ...
      console.log(
        `Win triggered with ${winScore} points (base + bonus + accumulated)`
      );
    ...
    case "loss":
      ...
      console.log(
        `Loss triggered with ${lossScore} points (base + bonus + accumulated)`
      );
